          ...
          <div className="p-4">
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                ...
                <TabList className="flex gap-2 bg-gray-100/50 backdrop-blur-sm p-1.5 rounded-lg">
                  {Object.entries(metricConfig).map(([key, config]) => (
                    <Tab 
                      key={key} 
                      className="px-3 py-1.5 text-sm font-medium rounded-md transition-all duration-200 
                        ui-not-selected:bg-transparent ui-not-selected:text-gray-600 
                        ui-selected:bg-white ui-selected:text-blue-600 ui-selected:shadow-sm
                        hover:text-blue-600 focus:outline-none relative
                        after:content-[''] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5
                        ui-selected:after:bg-blue-500"
                    >
                      <div className="flex items-center gap-2">
                        {config.title}
                        <div className="ui-selected:opacity-100 opacity-0 transition-opacity">
