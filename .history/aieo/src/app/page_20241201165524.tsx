...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }) => {
    ...
    const handleOptionClick = (level: string, option: string) => {
      ...
      // Get the current path up to the level being clicked
      const currentPathUpToLevel = path.slice(0, pathLengths[level as keyof typeof pathLengths] - 1);

      // Create new path by keeping the existing path and adding the new selection
      const newPath = [
        ...currentPathUpToLevel,
        { 
