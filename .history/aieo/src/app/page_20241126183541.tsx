...
export default function VisibilityDashboard() {
  ...
  const getCompetitorsForICP = (profile: string): { name: string; score: number }[] => {
    const competitorsByProfile: { [key: string]: { name: string; score: number }[] } = {
      'Enterprise DevOps Teams': [
        { name: 'Redgate', score: 82 },
        { name: 'Ariga.io', score: 78 },
        { name: 'PlanetScale', score: 75 },
        { name: 'Hasura', score: 72 },
        { name: 'Atlas', score: 68 }
      ],
      'Cloud-Native Startups': [
        { name: 'Ariga.io', score: 85 },
        { name: 'PlanetScale', score: 82 },
        { name: 'Hasura', score: 78 },
        { name: 'Atlas', score: 75 },
        { name: 'Redgate', score: 70 }
      ],
      'Financial Services': [
        { name: 'Redgate', score: 75 },
        { name: 'Atlas', score: 72 },
        { name: 'PlanetScale', score: 68 },
        { name: 'Ariga.io', score: 65 },
        { name: 'Hasura', score: 62 }
      ],
      'SaaS Providers': [
        { name: 'PlanetScale', score: 88 },
        { name: 'Hasura', score: 85 },
        { name: 'Ariga.io', score: 82 },
        { name: 'Atlas', score: 78 },
        { name: 'Redgate', score: 75 }
      ]
    };
                ...
                >
                  <summary className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50">
                    <div className="flex items-center gap-4">
                      ...
                      <Text className="text-xs text-gray-500">
                        {icp.overallScore >= 85 ? '✨ leading market' :
                         icp.overallScore >= 75 ? '📈 strong presence' :
                         icp.overallScore >= 65 ? '👀 growing impact' :
                         '🎯 target segment'}
                      </Text>
                    </div>
                    <svg 
                        ...
                        <div className="space-y-2">
                          {getCompetitorsForICP(icp.profile).map((competitor, index) => (
                            ...
                            >
                              ...
                              <Text className="text-xs font-medium">{competitor.score}%</Text>
                            </div>
                          ))}
                        </div>
                      </Card>
