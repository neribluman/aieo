...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }) => {
    ...
    const handleOptionClick = (level: string, option: string) => {
      ...
      // Create a new path with the selected option
      const newPath = [{ 
      ...
      // If we're not at the first level, include the previous path items
      if (level !== 'region') {
        newPath.unshift(...path.slice(0, pathLengths[level] - 1));
      }
