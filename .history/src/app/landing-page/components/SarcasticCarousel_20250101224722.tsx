...
export function SarcasticCarousel() {
  ...
  // Create 10 sets of companies for smoother infinite scroll
  const extendedCompanies = Array(10).fill(companies).flat();
      ...
      >
        <CarouselContent>
          {extendedCompanies.map((company, index) => (
            <CarouselItem key={`${company.name}-${index}`} className="basis-auto pl-6">
              <motion.div
