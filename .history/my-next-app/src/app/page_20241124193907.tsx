...
export default function VisibilityDashboard() {
  ...
  const sampleQueries = [
    ...
    },
    {
      ...
      query: "schema migration automation tools",
      platforms: [
        { name: "Gemini", position: 1, cited: true },
        { name: "SearchGPT", position: 3, cited: true },
        { name: "Claude", position: 6, cited: false }
      ],
      impact: "High",
    ...
    },
    {
      ...
      query: "Ariga vs Prisma schema management",
      platforms: [
        { name: "Claude", position: 2, cited: true },
        { name: "Perplexity", position: "-", cited: false },
        { name: "Gemini", position: 5, cited: true }
      ],
      impact: "Medium",
    ...
    },
    {
      ...
      query: "database version control best practices",
      platforms: [
        { name: "Gemini", position: 3, cited: true },
        { name: "MetaAI", position: "-", cited: false },
        { name: "Perplexity", position: 7, cited: false }
      ],
      impact: "Medium",
    ...
    },
    {
      ...
      query: "schema-as-code implementation guide",
      platforms: [
        { name: "Perplexity", position: 1, cited: true },
        { name: "Claude", position: "-", cited: false },
        { name: "SearchGPT", position: 4, cited: true }
      ],
      impact: "High",
                  ...
                  <Table className="mt-4">
                    ...
                    <TableBody>
                      {sampleQueries.map((query) => {
                        const bestPosition = Math.min(...query.platforms
                          .map(p => p.position === "-" ? Infinity : parseInt(p.position)));
                        const citationRate = (query.platforms.filter(p => p.cited).length / query.platforms.length) * 100;
                        ...
                        return (
                          <TableRow key={query.query}>
                            ...
                            <TableCell>
                              <Badge color={bestPosition === Infinity ? "red" : 
                                             bestPosition <= 3 ? "green" : 
                                             bestPosition <= 5 ? "yellow" : "orange"}>
                                {bestPosition === Infinity ? "Not Listed" : `#${bestPosition}`}
                              </Badge>
                            </TableCell>
                            <TableCell>
                              <Badge color={citationRate >= 75 ? "green" : citationRate >= 50 ? "yellow" : "red"}>
                      ...
                      {sampleQueries.flatMap((query) =>
                        query.platforms.map((platform, idx) => (
                          <TableRow key={`${query.query}-${platform.name}`}>
                            ...
                            <TableCell>
                              <Badge color={platform.position === "-" ? "red" : 
                                             parseInt(platform.position) <= 3 ? "green" : 
                                             parseInt(platform.position) <= 5 ? "yellow" : "orange"}>
