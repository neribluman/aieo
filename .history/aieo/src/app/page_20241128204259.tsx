...
export default function VisibilityDashboard() {
  ...
  // Update the transformDataForChart function to format dates consistently
  const transformDataForChart = (metric: typeof selectedMetric) => {
    const groupedData = aiEngineTimeSeries.reduce((acc, curr) => {
      ...
      if (dateEntry) {
        dateEntry[curr.engine] = Number(curr[metric]);  // Ensure numeric values
      } else {
        acc.push({
          date: curr.date,
          [curr.engine]: Number(curr[metric])
        });
      }
      return acc;
