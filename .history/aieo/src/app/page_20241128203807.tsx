...
export default function VisibilityDashboard() {
  ...
  const transformDataForChart = (metric: typeof selectedMetric) => {
    // Group by date and create an object with engine values for the selected metric
    const groupedData = aiEngineTimeSeries.reduce((acc, curr) => {
      const dateEntry = acc.find(item => item.date === curr.date);
      if (dateEntry) {
        dateEntry[curr.engine] = curr[metric];
      } else {
        acc.push({
          date: curr.date,
          [curr.engine]: curr[metric]
        });
      }
      return acc;
    ...
    console.log('Chart data for metric:', metric, sortedData);
    return sortedData;
  };
                            ...
                            <div className="p-3 rounded-lg bg-amber-50">
                              <div className="flex items-center gap-2 text-amber-700">
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  ...
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 12 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
