...
function BuyingJourneyFunnel() {
  ...
  // Update the JourneyMap component to pass options
  const JourneyMap = ({ path, onNodeClick }) => {
    // ... existing getNodeData function ...

    const handleOptionClick = (level: string, option: string) => {
      // Find the appropriate path length for this level
      const pathLengths = {
        region: 1,
        vertical: 2,
        persona: 3,
      ...
      // Create a new path up to this level with the selected option
          ...
          <div className="relative flex items-center">
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center">
                <svg 
                  className={`w-6 h-6 ${
                    path.length === 4 ? 'text-blue-500' : 
                    path.length > 3 ? 'text-gray-600' : 
                    'text-gray-300'
                  }`}
                  fill="none" 
                  viewBox="0 0 24 24" 
