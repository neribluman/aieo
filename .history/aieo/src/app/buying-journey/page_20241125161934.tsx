...
// Add a helper function for stage colors
function getStageGradient(stage: EnhancedFunnelStage, index: number) {
      ...
      <div className="flex justify-center items-center h-64 w-full">
        <svg viewBox="-100 -100 200 200" className="w-full h-full max-w-64">
          {data.map((item, index) => {
            ...
            const labelPos = calculateCirclePosition(
              index + 0.5,
              data.length,
              50
            );
          ...
          >
            ...
            >
              <div className="flex items-center gap-4">
                ...
                {/* Funnel bar */}
                <div
                  className={`h-24 bg-gradient-to-r ${getStageGradient(stage, index)} rounded-lg relative 
            ...
            <AnimatePresence>
              {selectedStage === stage.stage && (
                ...
                >
                  ...
                  <Card>
                    <Title>Sentiment Context</Title>
                    <div className="mt-4 space-y-6">
                      <div>
                        <Text className="font-medium mb-2">Top Positive Triggers</Text>
                        <div className="space-y-2">
                          {[
                            { trigger: "Technical capabilities", count: Math.round(stage.sentiment.positive * 0.4) },
                            { trigger: "Modern architecture", count: Math.round(stage.sentiment.positive * 0.3) },
                            { trigger: "Developer experience", count: Math.round(stage.sentiment.positive * 0.3) }
                          ].map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                              <div className="flex-grow">
                                <div className="flex justify-between mb-1">
                                  <Text className="text-sm">{item.trigger}</Text>
                                  <Text className="text-sm text-emerald-600">{item.count}%</Text>
                                </div>
                      ...
                      <div>
                        <Text className="font-medium mb-2">Main Concerns</Text>
                        <div className="space-y-2">
                          {[
                            { concern: "Enterprise proof points", count: Math.round(stage.sentiment.negative * 0.5) },
                            { concern: "Implementation complexity", count: Math.round(stage.sentiment.negative * 0.3) },
                            { concern: "Documentation gaps", count: Math.round(stage.sentiment.negative * 0.2) }
                          ].map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2">
                              <div className="flex-grow">
                                <div className="flex justify-between mb-1">
                                  <Text className="text-sm">{item.concern}</Text>
                                  <Text className="text-sm text-rose-600">{item.count}%</Text>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-1.5">
                                  <div
