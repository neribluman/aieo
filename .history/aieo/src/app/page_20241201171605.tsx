...
function BuyingJourneyFunnel() {
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-8">
                    {platforms.map(platform => (
                      <Text key={platform} className="text-sm text-gray-600 w-24 text-center">{platform}</Text>
                    ))}
                  </div>
                </div>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-8">
                              {platforms.map(platform => {
                                ...
                                return (
                                  <div key={platform} className="w-24 text-center">
            ...
            {options.map((option, index) => (
              <button
                ...
                className={`text-left text-sm hover:text-blue-600 transition-colors ${
                  isActive || isFuture ? 'text-gray-600' : 'text-gray-500'
                }`}
              >
                {option}
  ...
  // Update the JourneyMap component to include getNodeData
  const JourneyMap = ({ path, onNodeClick }) => {
      ...
      switch (level) {
        case 'region':
          return {
            ...
            metric: { value: 42 },
            options: ['Americas', 'EMEA'] // Only show actual regions we have data for
          };
        case 'vertical':
          return {
    ...
    // Inside JourneyMap component, update the handleOptionClick function:
    const handleOptionClick = (level: string, option: string) => {
      // Prevent event bubbling
      event?.stopPropagation();

      // Get current path up to the clicked level
      ...
      // If we're clicking at the current level or next level, append
      if (path.length === currentLevelIndex) {
        handleNavigation(null, [...path, newNode]);
      }
      // If we're clicking on a past level, rebuild path up to that point
      else if (path.length > currentLevelIndex) {
        const newPath = [...path.slice(0, currentLevelIndex), newNode];
        handleNavigation(null, newPath);
      }
    };
      ...
      <div className="bg-gradient-to-br from-gray-50 via-white to-blue-50/20 backdrop-blur-sm rounded-xl p-4 mb-6 shadow-sm">
        <div className="flex items-start gap-6 overflow-x-auto pb-2">
          ...
          <JourneyNode
            ...
            options={path.length === 0 ? getNodeData('region').options : undefined}
            metric={getNodeData('region').metric}
            isActive={path.length === 0}
            isPast={path.length > 0}
            isFuture={false}
            onOptionClick={(option) => handleOptionClick('region', option)}
            onClick={() => path.length > 0 ? onNodeClick(0) : null} // Only allow clicking if we have a path
          />
          ...
          <JourneyNode
            ...
            options={path.length === 1 ? getNodeData('vertical').options : undefined}
            metric={path.length > 0 ? getNodeData('vertical').metric : undefined}
            isActive={path.length === 1}
            isPast={path.length > 1}
            isFuture={path.length === 0}
            onOptionClick={(option) => handleOptionClick('vertical', option)}
            onClick={() => path.length > 1 ? onNodeClick(1) : null}
          />
          ...
          <JourneyNode
            ...
            options={path.length === 2 ? getNodeData('persona').options : undefined}
            metric={path.length > 1 ? getNodeData('persona').metric : undefined}
            isActive={path.length === 2}
            isPast={path.length > 2}
            isFuture={path.length < 2}
            onOptionClick={(option) => handleOptionClick('persona', option)}
            onClick={() => path.length > 2 ? onNodeClick(2) : null}
          />
          ...
          <JourneyNode
            ...
            options={path.length === 3 ? getNodeData('queries').options : undefined}
            metric={path.length > 2 ? getNodeData('queries').metric : undefined}
            isActive={path.length === 3}
            isPast={path.length > 3}
            isFuture={path.length < 3}
            onOptionClick={(option) => handleOptionClick('queries', option)}
            onClick={() => path.length > 3 ? onNodeClick(3) : null}
          />

          {/* Query Details */}
          <JourneyNode
            title={getNodeData('queryDetails').title}
            subtitle={path.length > 4 ? getCurrentLevelTitle('queryDetails') : undefined}
            options={path.length === 4 ? getNodeData('queryDetails').options : undefined}
            metric={path.length > 3 ? getNodeData('queryDetails').metric : undefined}
            isActive={path.length === 4}
            isPast={false}
            isFuture={path.length < 4}
            onOptionClick={(option) => handleOptionClick('queryDetails', option)}
            onClick={() => path.length > 4 ? onNodeClick(4) : null}
          />
        </div>
      </div>
    );
  ...
  const competitorData: CompetitorData[] = [
    {
      ...
      topSources: [
        { 
          name: "Database DevOps Report 2024: Key Findings and Trends",
          url: "https://www.infoq.com/articles/database-devops-2024",
          platforms: ["Claude", "Perplexity", "Gemini"],
          citationRate: 4.2
        },
        { 
          name: "How Redgate Simplified Schema Management for Fortune 500",
          url: "https://thenewstack.io/redgate-schema-management-case-study",
