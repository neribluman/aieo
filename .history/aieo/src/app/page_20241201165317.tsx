...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }) => {
    ...
    const handleOptionClick = (level: string, option: string) => {
      ...
      // Create a new path that includes the current path up to this level
      // and adds the new selection
      let newPath;
      if (level === 'region') {
        ...
        // For other levels, keep the existing path and add the new selection
        newPath = [
          ...path.slice(0, pathLengths[level as keyof typeof pathLengths] - 1),
          { 
            id: option.toLowerCase(), 
            name: option, 
            level 
