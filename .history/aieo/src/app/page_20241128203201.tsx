...
export default function VisibilityDashboard() {
  ...
  // Update the transformDataForChart function
  const transformDataForChart = (metric: typeof selectedMetric) => {
    const dates = ['2024-01', '2024-02', '2024-03', '2024-04', '2024-05'];
    const engines = ['Perplexity', 'Claude', 'Gemini', 'SearchGPT', 'MetaAI'];
    
    const result = dates.map(date => {
      const dataPoint: Record<string, any> = { date };
      engines.forEach(engineName => {
        const engineData = aiEngineTimeSeries.find(
          item => item.date === date && item.engine === engineName
        );
        // Make sure we're getting numbers
        dataPoint[engineName] = engineData ? Number(engineData[metric]) : 0;
      });
      return dataPoint;
    });

    console.log('Transformed data:', result); // Add this to debug
    return result;
  };
                            ...
                            <div className="p-3 rounded-lg bg-amber-50">
                              <div className="flex items-center gap-2 text-amber-700">
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  ...
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
