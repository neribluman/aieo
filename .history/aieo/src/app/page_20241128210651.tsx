              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <TabList className="gap-2">
                  {Object.entries(metricConfig).map(([key, config]) => (
                    <Tab 
                      key={key} 
                      className="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ui-selected:bg-blue-500 ui-selected:text-white ui-not-selected:bg-gray-100 ui-not-selected:text-gray-600 hover:bg-blue-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-300"
                    >
                      {config.title}
                    </Tab>
                  ))}
                ...
                {Object.entries(metricConfig).map(([key, config]) => (
                  <TabPanel key={key}>
                    <LineChart
                      ...
                      showLegend={true}
                      curveType="monotone"
                      showGridLines={true}  // Changed to true
                      showXAxis={true}
                      showYAxis={true}
                      gridLineColor="gray"  // Added this
