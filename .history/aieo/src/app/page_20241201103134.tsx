                      ...
                      <div className="p-4 border-t border-gray-200 bg-gray-50">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                              {competitor.topSources.map((source, idx) => (
                                <div key={idx} className="flex items-center justify-between">
                                  ...
                                  <Text className="text-xs">{source.citationRate}%</Text>
                                </div>
                              ))}
                            </div>
                          </Card>
                          ...
                          <Card decoration="left" decorationColor="violet" className="bg-white/60">
                            <Text className="text-sm font-medium mb-2">How You Compare</Text>
                            <div className="space-y-4">
                              <table className="w-full">
                                <thead>
                                  <tr>
                                    <th className="text-left text-sm text-gray-500">Category</th>
                                    <th className="text-right text-sm text-gray-500">Ariga.io</th>
                                    <th className="text-right text-sm text-gray-500">{competitor.company}</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {/* Overall */}
                                  <tr className="border-b border-gray-100 bg-gray-50">
                                    <td className="py-3 text-sm font-medium">Overall Company Mentioned</td>
                                    <td className="text-right text-sm font-medium">{32}%</td>
                                    <td className="text-right text-sm font-medium">{competitor.visibilityProbability}%</td>
                                  </tr>

                                  {/* Regional Category */}
                                  <tr className="border-b border-gray-100 bg-blue-50">
                                    <td colSpan={3} className="py-3 text-sm font-medium">Regional</td>
                                  </tr>
                                  {competitor.demographics.topRegions.map((region, idx) => (
                                    <tr key={`region-${idx}`} className="border-b border-gray-100">
                                      <td className="py-2 text-sm pl-4">{region.name}</td>
                                      <td className="text-right text-sm">{region.score - 5}%</td>
                                      <td className="text-right text-sm">{region.score}%</td>
                                    </tr>
                                  ))}

                                  {/* Vertical Category */}
                                  <tr className="border-b border-gray-100 bg-blue-50">
                                    <td colSpan={3} className="py-3 text-sm font-medium">Vertical</td>
                                  </tr>
                                  {competitor.demographics.topVerticals.map((vertical, idx) => (
                                    <tr key={`vertical-${idx}`} className="border-b border-gray-100">
                                      <td className="py-2 text-sm pl-4">{vertical.name}</td>
                                      <td className="text-right text-sm">{vertical.score - 4}%</td>
                                      <td className="text-right text-sm">{vertical.score}%</td>
                                    </tr>
                                  ))}

                                  {/* Persona Category */}
                                  <tr className="border-b border-gray-100 bg-blue-50">
                                    <td colSpan={3} className="py-3 text-sm font-medium">Persona</td>
                                  </tr>
                                  {competitor.demographics.topICPs.map((icp, idx) => (
                                    <tr key={`icp-${idx}`} className="border-b border-gray-100">
                                      <td className="py-2 text-sm pl-4">{icp.name}</td>
                                      <td className="text-right text-sm">{icp.score - 3}%</td>
                                      <td className="text-right text-sm">{icp.score}%</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
