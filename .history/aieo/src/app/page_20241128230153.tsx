...
function BuyingJourneyFunnel() {
  ...
  // Define sample queries first
  const sampleQueries: QueryPerformance[] = [
    {
      id: 'q1',
  ...
  const regions: FunnelNode[] = [
    ...
    },
    {
      ...
      metrics: [
        ...
        { label: 'Average Position', value: 3.2, trend: '+0.8' },
        { label: 'Company Mentioned', value: 35, trend: '+8%' },
        { label: 'Recommendation Probability', value: 45, trend: '+14%' }
      ],
      details: {
        topMarkets: ['Japan', 'Australia', 'Singapore']
      }
    }
  ];

  // Update personas definition to use sampleQueries
  const personas: FunnelNode[] = [
    {
      ...
      queries: sampleQueries.filter(q => q.category === "Problem Recognition").slice(0, 5)
    },
    {
      id: 'db-architect',
      ...
      queries: sampleQueries.filter(q => q.category === "Research").slice(0, 5)
    },
    // ... other personas
  ];

  // Update verticals to match FunnelNode type
        ...
        >
          ...
          {persona.queries && (
            <div className="mt-4 pt-4 border-t">
              <Text className="text-sm text-gray-600">View Related Queries ({persona.queries.length})</Text>
              <ChevronRightIcon className="w-5 h-5 text-gray-400" />
            </div>
          )}
        </Card>
  ...
  const handleBack = () => {
    ...
    if (previousLevel.level === 'region') {
      previousNode = regions.find(r => r.id === previousLevel.id) || null;
