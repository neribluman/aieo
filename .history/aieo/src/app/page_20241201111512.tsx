...
function BuyingJourneyFunnel() {
  ...
  const JourneyNode = ({ 
    ...
    <button
      onClick={onClick}
      className={`
        relative group flex flex-col min-w-[180px] h-[120px] p-3 rounded-xl border-2 transition-all
        justify-between
        ${isActive ? 
    ...
    >
      ...
      {(isActive || isPast) && (
        <div className="absolute -right-6 top-1/2 -translate-y-1/2 w-12">
          <div className="h-[2px] bg-gradient-to-r from-blue-200 via-blue-100 to-transparent">
            {/* Animated particles */}
            <div className="absolute inset-0">
              <div className="absolute h-full w-2 bg-blue-400/30 animate-flow-1" />
              <div className="absolute h-full w-2 bg-blue-400/30 animate-flow-2" />
              <div className="absolute h-full w-2 bg-blue-400/30 animate-flow-3" />
            </div>
          </div>
        </div>
      )}
