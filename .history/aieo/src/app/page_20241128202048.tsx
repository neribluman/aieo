...
export default function VisibilityDashboard() {
  ...
  // Add this new data structure for the time series
  const aiEngineTimeSeries: AIEngineTimeSeriesData[] = [
    // Perplexity
    { date: '2024-01', engine: 'Perplexity', avgSentiment: 0.82, avgPosition: 2.5, visibilityRate: 45, recommendationProb: 42 },
    { date: '2024-02', engine: 'Perplexity', avgSentiment: 0.84, avgPosition: 2.3, visibilityRate: 48, recommendationProb: 44 },
    { date: '2024-03', engine: 'Perplexity', avgSentiment: 0.85, avgPosition: 2.2, visibilityRate: 50, recommendationProb: 46 },
    { date: '2024-04', engine: 'Perplexity', avgSentiment: 0.87, avgPosition: 2.0, visibilityRate: 52, recommendationProb: 48 },
    { date: '2024-05', engine: 'Perplexity', avgSentiment: 0.88, avgPosition: 1.8, visibilityRate: 55, recommendationProb: 50 },
    
    // Claude
    { date: '2024-01', engine: 'Claude', avgSentiment: 0.78, avgPosition: 3.0, visibilityRate: 36, recommendationProb: 35 },
    { date: '2024-02', engine: 'Claude', avgSentiment: 0.80, avgPosition: 2.8, visibilityRate: 38, recommendationProb: 37 },
    { date: '2024-03', engine: 'Claude', avgSentiment: 0.81, avgPosition: 2.7, visibilityRate: 40, recommendationProb: 38 },
    { date: '2024-04', engine: 'Claude', avgSentiment: 0.83, avgPosition: 2.5, visibilityRate: 42, recommendationProb: 40 },
    { date: '2024-05', engine: 'Claude', avgSentiment: 0.85, avgPosition: 2.3, visibilityRate: 45, recommendationProb: 42 },
    
    // Gemini
    { date: '2024-01', engine: 'Gemini', avgSentiment: 0.75, avgPosition: 2.8, visibilityRate: 38, recommendationProb: 40 },
    { date: '2024-02', engine: 'Gemini', avgSentiment: 0.77, avgPosition: 2.6, visibilityRate: 40, recommendationProb: 41 },
    { date: '2024-03', engine: 'Gemini', avgSentiment: 0.79, avgPosition: 2.5, visibilityRate: 42, recommendationProb: 43 },
    { date: '2024-04', engine: 'Gemini', avgSentiment: 0.81, avgPosition: 2.3, visibilityRate: 44, recommendationProb: 44 },
    { date: '2024-05', engine: 'Gemini', avgSentiment: 0.83, avgPosition: 2.1, visibilityRate: 46, recommendationProb: 46 },
    
    // SearchGPT
    { date: '2024-01', engine: 'SearchGPT', avgSentiment: 0.72, avgPosition: 3.2, visibilityRate: 32, recommendationProb: 38 },
    { date: '2024-02', engine: 'SearchGPT', avgSentiment: 0.74, avgPosition: 3.0, visibilityRate: 34, recommendationProb: 39 },
    { date: '2024-03', engine: 'SearchGPT', avgSentiment: 0.76, avgPosition: 2.8, visibilityRate: 36, recommendationProb: 41 },
    { date: '2024-04', engine: 'SearchGPT', avgSentiment: 0.78, avgPosition: 2.6, visibilityRate: 38, recommendationProb: 42 },
    { date: '2024-05', engine: 'SearchGPT', avgSentiment: 0.80, avgPosition: 2.4, visibilityRate: 40, recommendationProb: 44 },
    
    // MetaAI
    { date: '2024-01', engine: 'MetaAI', avgSentiment: 0.70, avgPosition: 4.2, visibilityRate: 24, recommendationProb: 28 },
    { date: '2024-02', engine: 'MetaAI', avgSentiment: 0.71, avgPosition: 4.0, visibilityRate: 25, recommendationProb: 29 },
    { date: '2024-03', engine: 'MetaAI', avgSentiment: 0.73, avgPosition: 3.8, visibilityRate: 26, recommendationProb: 30 },
    { date: '2024-04', engine: 'MetaAI', avgSentiment: 0.74, avgPosition: 3.6, visibilityRate: 27, recommendationProb: 31 },
    { date: '2024-05', engine: 'MetaAI', avgSentiment: 0.76, avgPosition: 3.4, visibilityRate: 28, recommendationProb: 32 },
  ];
  ...
  // Inside the VisibilityDashboard component, before the return statement
  // Add this data transformation function
  const transformDataForChart = (metric: typeof selectedMetric) => {
    // Get unique dates
    const dates = [...new Set(aiEngineTimeSeries.map(item => item.date))];
    
    // Create transformed data array
    return dates.map(date => {
      // Get all entries for this date
      const dateEntries = aiEngineTimeSeries.filter(item => item.date === date);
      
      // Create an object with the date and all engine values for the selected metric
      return {
        date,
        Perplexity: dateEntries.find(entry => entry.engine === 'Perplexity')?.[metric] ?? 0,
        Claude: dateEntries.find(entry => entry.engine === 'Claude')?.[metric] ?? 0,
        Gemini: dateEntries.find(entry => entry.engine === 'Gemini')?.[metric] ?? 0,
        SearchGPT: dateEntries.find(entry => entry.engine === 'SearchGPT')?.[metric] ?? 0,
        MetaAI: dateEntries.find(entry => entry.engine === 'MetaAI')?.[metric] ?? 0,
      };
    });
  };
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <TabList className="gap-2">
                  {Object.entries(metricConfig).map(([key, config]) => (
                    <Tab key={key} className="px-4 py-2">
                      {config.title}
                    </Tab>
                  ))}
                </TabList>
                              ...
                              {payload.map((entry: any, index: number) => (
                                <div key={index} className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm">
                                    {entry.dataKey}: {config.valueFormatter(entry.value)}
                                  </span>
                                </div>
                              ))}
                            </div>
