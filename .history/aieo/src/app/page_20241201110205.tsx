...
function BuyingJourneyFunnel() {
    ...
    >
      ...
      {metric && (
        <div className="flex items-center gap-1 mt-1">
          ...
          <Text className="text-sm font-medium text-gray-700">{metric.value}</Text>
        </div>
      )}
      {(isActive || isPast) && (
        <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-[2px] bg-gray-200" />
      )}
    </button>
  );
  ...
  const JourneyMap = ({ 
    ...
    const getNodeMetric = (node: BreadcrumbItem) => {
      switch (node.level) {
        case 'region':
          return { label: 'Visibility', value: '42%' };
        case 'vertical':
          return { label: 'Market Share', value: '28%' };
        case 'persona':
          return { label: 'Queries', value: '124' };
        default:
          return undefined;
      }
    };
      ...
      <div className="bg-white/50 backdrop-blur-sm rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          <JourneyNode
            ...
            metric={{ label: 'Total Regions', value: '4' }}
          ...
          {path.map((item, index) => (
            <div key={item.id} className="flex items-center">
              <JourneyNode
                title={item.name}
                metric={getNodeMetric(item)}
                isActive={index === path.length - 1}
                isPast={index < path.length - 1}
          ...
          {path.length < 3 && (
            <div className="flex items-center">
              <JourneyNode
                title={
                  path.length === 0 ? 'Select Region' :
                  path.length === 1 ? 'Select Vertical' :
                  'Select Persona'
                }
                isActive={false}
                isPast={false}
