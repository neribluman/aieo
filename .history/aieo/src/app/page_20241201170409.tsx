...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }) => {
    ...
    const handleOptionClick = (level: string, option: string) => {
      ...
      // Create new path based on the level and current path
      let newPath;
      if (level === 'region') {
        ...
        // Get the current path up to the previous level
        const previousPath = path.slice(0, pathLengths[level as keyof typeof pathLengths] - 1);
