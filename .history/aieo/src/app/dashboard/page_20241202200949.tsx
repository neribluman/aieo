...
import { Card, Title, Text, Metric, Flex, TabGroup, TabList, Tab, TabPanels, TabPanel, Badge } from '@tremor/react';
  ...
  const transformDataForChart = (metric: typeof selectedMetric) => {
    ...
    return uniqueDates.map(date => {
      ...
      entries.forEach(entry => {
        dataPoint[entry.engine as keyof Omit<ChartDataPoint, 'date'>] = entry[metric];
      });
    ...
    <div className="p-8 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 space-y-4">
          <h1 className={cn(
            "text-4xl font-bold tracking-tight lg:text-5xl text-center bg-gradient-to-r from-gray-900 via-blue-800 to-gray-900",
            "bg-clip-text text-transparent animate-gradient-x"
          )}>
            Ariga.io AI Visibility Dashboard
          </h1>
          <p className="text-xl text-center text-muted-foreground">
            Database Schema-as-Code Platform
          </p>
          <div className="h-1 w-24 mx-auto bg-gradient-to-r from-blue-500 to-blue-800 rounded-full" />
        </div>
          ...
          <Tabs value={selectedMetric} onValueChange={(value) => setSelectedMetric(value as typeof selectedMetric)}>
            <CardHeader>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
                <CardTitle>AI Engine Performance Over Time</CardTitle>
                <TabsList>
              ...
              <TabsContent value={selectedMetric}>
                <div className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart 
                      data={transformDataForChart(selectedMetric)}
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        tick={{ fill: 'hsl(var(--muted-foreground))' }}
                        tickFormatter={(value) => 
                          selectedMetric === 'avgPosition' 
                            ? `#${value.toFixed(1)}` 
                            : selectedMetric === 'avgSentiment'
                            ? `${(value * 100).toFixed(0)}%`
                            : `${value}%`
                        }
                      />
                                ...
                                {payload.map((item: any) => (
                                  <div key={item.name} className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium">
                                      {item.name}: {metricConfig[selectedMetric].valueFormatter(Number(item.value))}
                                    </span>
                                  </div>
                                ))}
                              </div>
                      ...
                      />
                      {['Perplexity', 'Claude', 'Gemini', 'SearchGPT', 'AIO'].map((engine, index) => (
                        <Line
                          key={engine}
                          type="monotone"
                          dataKey={engine}
