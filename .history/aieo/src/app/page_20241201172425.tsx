...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }: { path: BreadcrumbItem[]; onNodeClick: (index: number) => void }) => {
    const handleOptionClick = (e: React.MouseEvent, level: string, option: string) => {
      ...
      // Build the new path based on the current level
      let newPath: BreadcrumbItem[] = [];
      
      switch(level) {
        case 'region':
          // For region, start fresh
          newPath = [newNode];
          break;
        case 'vertical':
          // For vertical, keep the region and add this
          if (path.length >= 1) {
            newPath = [...path.slice(0, 1), newNode];
          }
          break;
        case 'persona':
          // For persona, keep region and vertical and add this
          if (path.length >= 2) {
            newPath = [...path.slice(0, 2), newNode];
          }
          break;
        case 'queries':
          // For queries, keep previous levels and add this
          if (path.length >= 3) {
            newPath = [...path.slice(0, 3), newNode];
          }
          break;
        case 'queryDetails':
          // For query details, keep all previous levels and add this
          if (path.length >= 4) {
            newPath = [...path.slice(0, 4), newNode];
          }
          break;
      }
      ...
      <div className="bg-gradient-to-br from-gray-50 via-white to-blue-50/20 backdrop-blur-sm rounded-xl p-4 mb-6 shadow-sm">
        <div className="flex items-start gap-6 overflow-x-auto pb-2">
          ...
          <JourneyNode
            ...
            subtitle={path.length > 0 ? path[0].name : undefined}
            options={path.length === 0 ? ['Americas', 'EMEA'] : undefined}
            metric={{ value: 42 }}
            isActive={path.length === 0}
            isPast={path.length > 0}
            isFuture={false}
            onOptionClick={(e, option) => handleOptionClick(e, 'region', option)}
            onClick={() => path.length > 0 && onNodeClick(0)}
          />
          
          {/* Industry Verticals */}
          <JourneyNode
            title="Industry Verticals"
            subtitle={path.length > 1 ? path[1].name : undefined}
            options={path.length === 1 ? ['Enterprise Software', 'Financial Services', 'Healthcare Tech'] : undefined}
            metric={{ value: 38 }}
            isActive={path.length === 1}
            isPast={path.length > 1}
            isFuture={path.length === 0}
            onOptionClick={(e, option) => handleOptionClick(e, 'vertical', option)}
            onClick={() => path.length > 1 && onNodeClick(1)}
          />

          {/* Buyer Personas */}
          <JourneyNode
            title="Buyer Personas"
            subtitle={path.length > 2 ? path[2].name : undefined}
            options={path.length === 2 ? ['DevOps Lead', 'Database Architect', 'Tech Lead'] : undefined}
            metric={{ value: 35 }}
            isActive={path.length === 2}
            isPast={path.length > 2}
            isFuture={path.length < 2}
            onOptionClick={(e, option) => handleOptionClick(e, 'persona', option)}
            onClick={() => path.length > 2 && onNodeClick(2)}
          />

          {/* Search Queries */}
          <JourneyNode
            title="Search Queries"
            subtitle={path.length > 3 ? path[3].name : undefined}
            options={path.length === 3 ? ['Schema migration tools', 'Database version control', 'CI/CD integration'] : undefined}
            metric={{ value: 32 }}
            isActive={path.length === 3}
            isPast={path.length > 3}
            isFuture={path.length < 3}
            onOptionClick={(e, option) => handleOptionClick(e, 'queries', option)}
            onClick={() => path.length > 3 && onNodeClick(3)}
          />

          {/* Query Details */}
          <JourneyNode
            title="Query Details"
            subtitle={path.length > 4 ? path[4].name : undefined}
            options={path.length === 4 ? ['Full Analysis'] : undefined}
            metric={{ value: 28 }}
            isActive={path.length === 4}
            isPast={false}
            isFuture={path.length < 4}
            onOptionClick={(e, option) => handleOptionClick(e, 'queryDetails', option)}
            onClick={() => path.length > 4 && onNodeClick(4)}
          />
        </div>
      </div>
    );
