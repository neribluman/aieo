...
interface QueryPerformance {
  ...
  userIntent: string;
  buyingJourney: "Problem Exploration" | "Solution Education" | "Solution Comparison" | "Solution Evaluation" | "Final Research";
...
// First, let's properly export the TransitionWrapper component
...
// Then, let's fix the query filtering with proper type annotations
const personaQueries: Record<string, QueryPerformance[]> = {
  'DevOps Lead': [
    {
      id: "dl1",
      query: "CI/CD pipeline database integration best practices",
      category: "DevOps",
      impact: "High",
  ...
  'Database Architect': [
    {
      id: "da1",
      query: "Schema migration automation security implications",
            ...
            <Card className="bg-white p-6">
              ...
              <div>
                ...
                <div className="space-y-1">
                  {queries
                    .filter((q: QueryPerformance) => {
                      switch (activeNode.name) {
                        case 'DevOps Lead':
                          return personaQueries['DevOps Lead'].map((pq: QueryPerformance) => pq.id).includes(q.id);
                        case 'Database Architect':
                          return personaQueries['Database Architect'].map((pq: QueryPerformance) => pq.id).includes(q.id);
                        case 'Tech Lead':
                          return personaQueries['Tech Lead'].map((pq: QueryPerformance) => pq.id).includes(q.id);
                        default:
                          return true;
                      }
                    })
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 flex-1">
                              ...
                              <div className="flex items-center gap-3 w-full">
                                <Text className="text-sm flex-1">{query.query}</Text>
                                <Badge 
                                  size="sm" 
                                  color={
                            ...
                            <div className="flex gap-4">
                              {platforms.map(platform => {
                                ...
                                return (
                                  <div key={platform} className="w-16 text-center">
                                    <Text className={`text-sm ${
                                      ...
                                      {platformData?.position === '-' ? 'N/A' : `#${platformData?.position}`}
                                      {platformData?.cited && '•'}
                                    </Text>
                                  </div>
                                );
