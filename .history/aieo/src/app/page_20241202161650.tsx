        ...
        <Card className="bg-white/50 backdrop-blur-sm shadow-glow mb-6">
          <CardHeader>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
              <CardTitle className="text-xl font-semibold">AI Engine Performance Over Time</CardTitle>
              <TabsList>
          ...
          <CardContent>
            {Object.entries(metricConfig).map(([key, config]) => (
              <TabsContent key={key} value={key}>
                <ChartContainer 
                  config={{
                    Perplexity: { label: 'Perplexity', color: 'hsl(var(--chart-1))' },
                    Claude: { label: 'Claude', color: 'hsl(var(--chart-2))' },
                    Gemini: { label: 'Gemini', color: 'hsl(var(--chart-3))' },
                    SearchGPT: { label: 'SearchGPT', color: 'hsl(var(--chart-4))' },
                    AIO: { label: 'AIO', color: 'hsl(var(--chart-5))' },
                  }}
                  className="h-96"
                >
                  <AreaChart
                  ...
                  >
                    ...
                    {platforms.map((platform) => (
                      <Area
                        ...
                        dataKey={platform}
                        type="natural"
                        fill={`var(--color-${platform.toLowerCase()})`}
                        fillOpacity={0.4}
