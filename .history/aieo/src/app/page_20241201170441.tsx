...
function BuyingJourneyFunnel() {
  ...
  const JourneyMap = ({ path, onNodeClick }) => {
    ...
    const handleOptionClick = (level: string, option: string) => {
      ...
      // Create new path based on the level
      let newPath;
      if (level === 'region') {
        ...
        // For other levels, keep the existing path up to the previous level
        // and add the new selection
        const currentLevel = pathLengths[level as keyof typeof pathLengths];
        const previousLevelPath = path.slice(0, currentLevel - 1);
