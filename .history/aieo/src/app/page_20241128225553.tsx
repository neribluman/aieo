...
// Update the standardized metrics format
const getStandardMetrics = (data: {
  avgSentiment: number;
  avgPosition: number;
  visibilityRate: number;
  recommendationProb: number;
}) => [
  {
    ...
    value: `${(data.avgSentiment * 100).toFixed(0)}%`,
    trend: '+12%',
    color: 'blue'
  },
  {
  ...
  },
  {
    ...
    value: `${data.visibilityRate}%`,
    trend: '+15%',
    color: 'violet'
  },
  {
    label: 'Recommendation',
    value: `${data.recommendationProb}%`,
    trend: '+10%',
    color: 'amber'
  }
];

// Update the geographicRegions data
const geographicRegions: FunnelLevel = {
  id: 'regions',
  name: 'Geographic Regions',
  metrics: getStandardMetrics({
    avgSentiment: 0.42,
    avgPosition: 3.2,
    visibilityRate: 38,
    recommendationProb: 42
  }),
  children: [
    {
      id: 'americas',
...
// Update the verticals data in renderVerticals
const verticals = [
  {
    id: 'enterprise',
  ...
  // ... similar updates for other verticals
];

// Update the personas data in renderPersonas
const personas = [
  {
    id: 'devops-lead',
    title: 'DevOps Lead',
    ...
    details: {
      ...
      queries: sampleQueries.filter(q => 
        q.category === "Problem Recognition" || 
        q.category === "Research"
      )
    }
  },
    ...
    if (currentLevel === 'persona') {
      return (
        <div className="space-y-6">
          ...
          {/* Replace Journey Stages with Query Performance */}
          <Card className="bg-white p-6">
            <Title className="text-xl mb-6">Query Performance</Title>
            <div className="space-y-4">
              {activeNode.details.queries?.map((query, index) => (
                <Card key={index} className="bg-white/50">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <Text className="font-medium mb-1">{query.query}</Text>
                      <div className="flex gap-2">
                        <Badge color="blue">{query.category}</Badge>
                        <Badge color="emerald">{query.impact}</Badge>
                        <Badge color="violet">{query.userIntent}</Badge>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    {query.platforms.map((platform, pIndex) => (
                      <div key={pIndex} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                        <Text className="text-sm">{platform.name}</Text>
                        <div className="flex items-center gap-2">
                          <Badge 
                            color={
                              platform.position === '-' ? 'red' :
                              Number(platform.position) <= 2 ? 'green' :
                              Number(platform.position) <= 4 ? 'yellow' : 'orange'
                            }
                          >
                            {platform.position === '-' ? 'Not Listed' : `#${platform.position}`}
                          </Badge>
                          {platform.cited && (
                            <div className="w-2 h-2 rounded-full bg-green-500" title="Cited" />
                          )}
                        </div>
                      </div>
                    ))}
