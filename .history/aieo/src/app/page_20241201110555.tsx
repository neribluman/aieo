...
function BuyingJourneyFunnel() {
  ...
  const JourneyNode = ({ 
    ...
    <button
      onClick={onClick}
      className={`
        relative group flex flex-col min-w-[180px] p-3 rounded-lg border transition-all
        ${isActive ? 'bg-blue-50 border-blue-200 shadow-md' : 
          isPast ? 'bg-gray-50 border-gray-200 hover:bg-blue-50/50' : 
          isFuture ? 'bg-gray-50/50 border-gray-100 hover:bg-blue-50/30' :
    ...
    >
      ...
      {previewCount && (
        <Text className="text-xs text-gray-400 mt-1">
          {previewCount} options available
        </Text>
      )}
      {(isActive || isPast) && (
        <div className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-[2px] bg-gradient-to-r from-blue-200 to-transparent" />
      )}
    </button>
  );

  // Update the JourneyMap component to show the full funnel preview
  const JourneyMap = ({ 
    path, 
    onNodeClick 
  }: { 
    ...
    // Get metrics and preview counts based on the current state
    const getNodeData = (level: 'region' | 'vertical' | 'persona') => {
      switch (level) {
        case 'region':
          return {
            metric: { value: 42 },
    ...
    const getCurrentLevelTitle = (level: 'region' | 'vertical' | 'persona') => {
      const currentItem = path.find(item => item.level === level);
      if (currentItem) return currentItem.name;
      
      const data = getNodeData(level);
      return `${data.options[0]} +${data.previewCount - 1}`;
    };

    return (
      <div className="bg-gradient-to-r from-gray-50 to-blue-50/20 backdrop-blur-sm rounded-lg p-4 mb-6">
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          {/* Geographic Regions - Always visible */}
          <JourneyNode
          ...
          {/* Show selected region or preview */}
          <JourneyNode
            title={getCurrentLevelTitle('region')}
            metric={path.length > 0 ? { value: 42 } : undefined}
          ...
          {/* Show selected vertical or preview */}
          <JourneyNode
            title={getCurrentLevelTitle('vertical')}
            metric={path.length > 1 ? { value: 38 } : undefined}
          ...
          {/* Show selected persona or preview */}
          <JourneyNode
            title={getCurrentLevelTitle('persona')}
            metric={path.length > 2 ? { value: 35 } : undefined}
            isActive={path.length === 3}
            isPast={false}
            isFuture={path.length <= 2}
            previewCount={path.length === 3 ? undefined : 124}
